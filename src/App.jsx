...
function PlanoSection({ color, user, dynamicTopics, profile }) {
  ...
  WEEKS.forEach(w => w.topics.forEach(t => {
    if (done.has(`${w.n}-${t.id}`)) { doneTopicsCount++; doneHoursCount += t.h; }
  }));
      ...
      <div style={{ marginBottom: 18, padding: "14px 16px", background: "#fafaf8", border: `1px solid ${T.borderCard}`, borderRadius: 6 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 11, fontFamily: "monospace", color: color }}>
            {doneTopicsCount}/{totalTopics} tarefas · {doneHoursCount}h/{totalHours}h
          </span>
        </div>
        ...
        return (
          <div key={w.n} style={{ ...S.decisionWrap(allDone ? "#10B981" : w.col), padding: 0, overflow: "hidden", marginBottom: 8 }}>
            ...
            <div onClick={() => tog(w.n)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: allDone ? "#F0FDF4" : "#fafaf8", cursor: "pointer", transition: "background 0.2s" }}>
              <span style={{ fontSize: 10, fontFamily: "monospace", color: allDone ? "#14532D" : w.col, minWidth: 60, fontWeight: 600, letterSpacing: "0.06em" }}>
                SEMANA {w.n}
              </span>
              <span style={{ flex: 1, fontSize: 12.5, color: allDone ? T.textMuted : T.textPrimary, textDecoration: allDone ? "line-through" : "none", transition: "all 0.2s" }}>
                {w.focus}
              </span>
              <span style={{ fontSize: 10, fontFamily: "monospace", color: allDone ? "#14532D" : T.textDisabled, flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 11, fontFamily: "monospace", color: allDone ? "#14532D" : w.col, minWidth: 26, textAlign: "right" }}>
                {w.h}h
              </span>
              ...
              <div style={{ padding: "0 14px 10px" }}>
                {w.topics.map((t, i) => {
                  ...
                  const badgeStyle = isDone
                    ...
                    : { flexShrink: 0, width: 32, height: 20, borderRadius: 4, background: `${ti.color}15`, border: `1px solid ${ti.color}44`, color: ti.color, fontSize: 10, fontFamily: "monospace", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 600, transition: "all 0.2s" };

                  return (
                    <div key={key} onClick={(e) => { e.stopPropagation(); togDone(key); }} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "8px 4px", borderBottom: i < w.topics.length - 1 ? `1px solid ${T.borderCard}` : "none", cursor: "pointer", borderRadius: 4, background: isDone ? "#fafffe" : "transparent", transition: "background 0.2s", margin: "0 -4px" }}>
                      
                      <div style={badgeStyle}>{t.wipr > 0 ? t.wipr : "rev"}</div>
                      
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12.5, color: isDone ? T.textDisabled : T.textPrimary, lineHeight: 1.35, textDecoration: isDone ? "line-through" : "none", transition: "all 0.2s" }}>
                          {t.nome || t.id}
                        </div>
                        <div style={{ fontSize: 10, color: isDone ? T.textDisabled : T.textMuted, marginTop: 1, fontFamily: "monospace", transition: "color 0.2s", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                          Eixo: {t.especialidade || 'Geral'}
                        </div>
                      </div>

                      <span style={{ fontSize: 9, padding: "2px 5px", borderRadius: 3, background: isDone ? "#F0FDF4" : ms.bg, border: `1px solid ${isDone ? "#BBF7D0" : ms.br}`, color: isDone ? "#14532D" : ms.tx, fontFamily: "monospace", fontWeight: 600, flexShrink: 0, transition: "all 0.2s" }}>
                        {isDone ? "✓ feito" : t.mode}
                      </span>
                      
                      <span style={{ fontSize: 11, fontFamily: "monospace", color: isDone ? T.textDisabled : T.textMuted, flexShrink: 0, minWidth: 22, textAlign: "right" }}>
                        {t.h}h
                      </span>
                    </div>
                  );
