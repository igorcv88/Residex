...
// ─── tier config (dark) ───────────────────────────────────────────────────────
const TIER_CONFIG = {
  critical: { label: "Crítico — IPR 80–100 · Estude em profundidade",        dot: "#EF4444", bg: "#EF444414", border: "#EF444435", text: "#fca5a5", shortLabel: "CRÍTICO" },
  high:     { label: "Alta prioridade — IPR 60–79 · Solidez obrigatória",    dot: "#F59E0B", bg: "#F59E0B14", border: "#F59E0B35", text: "#fcd34d", shortLabel: "ALTA"    },
  mid:      { label: "Média prioridade — IPR 40–59 · Pontos-chave",          dot: "#10B981", bg: "#10B98114", border: "#10B98135", text: "#6ee7b7", shortLabel: "MÉDIA"   },
  low:      { label: "Baixa prioridade — IPR <40 · Só se sobrar tempo",      dot: "#0EA5E9", bg: "#0EA5E914", border: "#0EA5E935", text: "#7dd3fc", shortLabel: "BAIXA"   },
};
...
export default function CronogramaResidex() {
  ...
  const trendColor = (t) => t === "crescente" ? "#6ee7b7" : t === "decrescente" ? "#fca5a5" : T_DIM;

  const handleSpecFilter = (s) => { setSpecFilter(prev => prev === s ? null : s); setTierFilter("all"); };
  const handleTierFilter = (t) => { setTierFilter(t); setSpecFilter(null); };
  ...
  const filterBtn = (active, accentColor) => ({
    ...
    background: active ? (accentColor ? `${accentColor}18` : BG3) : "transparent",
    color: active ? (accentColor || T_PRI) : T_DIM,
    transition: "all 0.15s",
  });

  return (
    <div style={{ background: BG, minHeight: "100vh", color: T_PRI, fontFamily: FONT }}>
      <style>{`
      ...
      {/* ── header ──────────────────────────────────────────────────────────── */}
      <div style={{ background: BG2, borderBottom: `1px solid ${BORDER}`, padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button
            onClick={() => navigate("/")}
            style={{ background: "transparent", border: "none", color: T_DIM, cursor: "pointer", fontSize: 13, fontFamily: MONO, padding: 0 }}
          >
            ← MedPanel
          </button>
          <span style={{ color: BORDER2 }}>|</span>
          <span style={{ fontSize: 11, fontFamily: MONO, color: "#6366F1", letterSpacing: "0.05em" }}>RESIDEX</span>
          <span style={{ fontSize: 14, color: T_SEC, fontFamily: FONT }}>Cronograma por IPR</span>
        </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 20px" }}>
        ...
        {/* ── subheader ───────────────────────────────────────────────────── */}
        <p style={{ fontSize: 12, fontFamily: MONO, color: T_DIM, marginBottom: 24 }}>
        ...
        <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 24 }}>
          {[
            { label: "Temas totais",    val: data.length,       color: T_MUT,    accent: null           },
            { label: "IPR Crítico 80+", val: counts.critical,   color: "#fca5a5", accent: "#EF4444"     },
            { label: "Alta prioridade", val: counts.high,       color: "#fcd34d", accent: "#F59E0B"     },
            { label: "Baixa / Ignorar", val: counts.low + counts.mid, color: "#7dd3fc", accent: "#0EA5E9" },
          ].map(s => (
            <div key={s.label} style={{
              ...
              <div style={{ fontSize: 10, fontFamily: MONO, color: T_DIM, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.label}</div>
              <div style={{ fontSize: 24, fontWeight: 500, color: s.color, fontFamily: FONT }}>{s.val}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap", marginBottom: 20 }}>
          {TIER_ORDER.map(t => (
            <div key={t} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 11, fontFamily: MONO, color: T_MUT }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: TIER_CONFIG[t].dot, flexShrink: 0 }} />
        ...
        {/* ── filters — tier ──────────────────────────────────────────────── */}
        <div className="filters-row" style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
          <button onClick={() => handleTierFilter("all")} style={filterBtn(tierFilter === "all" && !specFilter, null)}>
            Todos
          </button>
          {TIER_ORDER.map(t => (
            <button key={t} onClick={() => handleTierFilter(t)} style={filterBtn(tierFilter === t && !specFilter, TIER_CONFIG[t].dot)}>
        ...
        {/* ── filters — spec ──────────────────────────────────────────────── */}
        <div className="filters-row" style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
          {SPECS.map(s => (
            <button key={s} onClick={() => handleSpecFilter(s)} style={filterBtn(specFilter === s, null)}>
              {SPEC_LABELS[s]}
            </button>
          ))}
        </div>
        ...
        {/* ── count ───────────────────────────────────────────────────────── */}
        <div style={{ fontSize: 11, fontFamily: MONO, color: T_DIM, marginBottom: 16 }}>
          ...
          return (
            <div key={tier}>
              ...
              {items.map((d, i) => {
                ...
                return (
                  <div className="topic-card" key={i} style={{
                    display: "flex",
                    ...
                    <div style={{
                      ...
                      <span style={{ fontSize: 17, fontWeight: 500, color: tc.text, lineHeight: 1.2, fontFamily: FONT }}>{d.ipr}</span>
                      <span style={{ fontSize: 8, fontFamily: MONO, color: tc.text, opacity: 0.75 }}>{tc.shortLabel}</span>
                    </div>

                    {/* body */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, color: T_SEC, marginBottom: 5, fontFamily: FONT }}>{d.name}</div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
                        ...
                        <span style={{ fontSize: 11, fontFamily: MONO, color: T_DIM }}>{d.inst}</span>
                        <span style={{ fontSize: 11, fontFamily: MONO, color: T_MUT }}>{d.freq}</span>
                        <span style={{ fontSize: 11, fontFamily: MONO, color: trendColor(d.trend) }}>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "3rem", color: T_DIM, fontSize: 13, fontFamily: MONO }}>
